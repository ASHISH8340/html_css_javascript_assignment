...
const Genres: React.FC<IProps> = ({ handleGenre, genre }) => {
  const [data, setData] = useState<IGenres[]>([{ id: 0, name: 'All' }]);
  const [stateData, setStateData] = useState<'pending' | 'fulfilled' | 'rejected'>('pending');

  useEffect(() => {
    (async () => {
      try {
        setStateData('pending');
        const fetchedData = await fetchGenres();
        if (Array.isArray(fetchedData)) {
          setStateData('fulfilled');
          setData(prevState => [...prevState, ...fetchedData]);
        } else {
          throw new Error('fetchGenres did not return an array of IGenres.');
        }
      } catch {
        setStateData('rejected');
      }
